...
class Point:
	...
	def __init__(self, frame, contour, cx, cy, radius):
		...
		self.contour = contour
                ...
                self.cy = cy
                self.radius = radius
                (self.width, self.height, self.area) = self.measure_width_height_area(contour, radius)

	def __repr__(self):
		return "CX:" + str(self.cx) + "\tCY:" + str(self.cy) + "\tRadius:" + str(self.radius)
...
class MovingObject:
	...
	def __str__(self):
		x = ' '.join(str(self.frames[f]) for f in sorted(self.frames, key=self.frames.get))
		return x
	...
	def measure(self):
		...
		if len(frames)  > 3 :
			...
			avg_radius = np.average([cp.radius for cp in self.frames.itervalues()])
			avg_width = np.average([cp.width for cp in self.frames.itervalues()])
			avg_height = np.average([cp.height for cp in self.frames.itervalues()])
			avg_area = np.average([cp.area for cp in self.frames.itervalues()])

			return (num_frames, fx, fy, lx, ly, first_frame, last_frame, avg_radius, avg_width, avg_height, avg_area)
...
class ObjectDatabase:
	...
	def remove_MovingObjects(self, frame):
		...
		lost_mc = [mc for mc in self.prev_OverlapMovingObjects if mc.last_frame < frame ]
		new_mc = [mc for mc in self.prev_OverlapMovingObjects if mc.last_frame >= frame   and len(mc.frames) == 1]
		existing_mc = [mc for mc in self.prev_OverlapMovingObjects if mc.last_frame <= frame   and len(mc.frames) > 1]
	
		logging.debug("New MC: " + str(len(new_mc)) + "\t combining with Lost: " + str(len(lost_mc)))
		...
		for new in sorted(new_mc, key=lambda mc: mc.get_last_xy()[0]):
			...
			if closest_mc:
				logging.debug("Found Possible New Instance Stitches: " + str(cnt))
		...
		measure = [mc for mc in self.prev_OverlapMovingObjects if mc.last_frame < frame  - buffer and mc not in dont_track]
		...
		for m in measure:
			...
			x = m.measure()
			if x :
				(num_frames, fx, fy, lx, ly, first_frame, last_frame, avg_radius, avg_width, avg_height, avg_area) = m.measure()
				k = (lx, ly)
				v = (fx, m)
				d[k].append(v)
		...
		self.prev_OverlapMovingObjects = [mc for mc in self.prev_OverlapMovingObjects if mc.last_frame >= frame - buffer and mc not in dont_track]
	...
	def add_contours(self, frame, orig_contours):
		...
		if len(self.prev_OverlapMovingObjects) == 0:
			logging.debug("Empty Moving Objects - Adding: " + str(len(order_contours)))
			...
			logging.debug("Checking New Overlap Contour: " + str(len(order_contours)) + " against MC: " + str(len(self.prev_OverlapMovingObjects)))
			for (cx,cnt) in sorted(order_contours, key=lambda tup: tup[0], reverse=False):
		
				matching_objects = [mc for mc in self.prev_OverlapMovingObjects if mc.match_overlap(cnt) ]
			...
			for mc in sorted(self.prev_OverlapMovingObjects, key=lambda x: x.get_last_xy()[0], reverse=False):
				...
				matching_cnts = sorted([(cx,cnt) for (cx,cnt) in order_contours if mc.match_overlap(cnt)], key=lambda t: t[0], reverse=True)
...
def parse_args(argv):
	...
	p.add_option('-v', '--visualize', action='store_true',  help='visualize tracking')
	p.add_option('-s', '--subtractbg', action='store_true',  help='subtract background')
...
def main(argv=None):
	...
	if opts.output :
		output_writer.open(opts.output, fourcc, 21.37, (int(video_width),int(video_height)), True)
		...
		for mc in reversed(object_db.prev_OverlapMovingObjects):
			if len(mc.frames.keys()) > 0:
				for f in mc.frames.keys():
					cp = mc.frames[f]
					cv2.circle(img,(int(cp.cx),int(cp.cy)),int(cp.radius),mc.color,1)
			if len(mc.frames.keys()) == 1:
				cv2.drawContours(img,[mc.get_last_contour()],-1,(0,255,0),-1,1)
